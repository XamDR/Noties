package net.azurewebsites.eznotes.ui.views
...
// Based on: https://github.com/MikeOrtiz/TouchImageView/blob/master/touchview/src/main/java/com/ortiz/touchview/TouchImageView.kt
...
class PhotoImageView @JvmOverloads constructor(
	...
	private var actionState: ImageActionState? = null
	...
	private var delayedZoom: Zoom? = null
	...
	init {
		...
		setState(ImageActionState.NORMAL)
		onDrawReady = false
		super.setOnTouchListener(PrivateOnTouchListener())
		val attributes = context.theme.obtainStyledAttributes(attrs, R.styleable.PhotoImageView, defStyle, 0)
		try {
			if (!isInEditMode) {
				isZoomEnabled = attributes.getBoolean(R.styleable.PhotoImageView_zoom_enabled, true)
			}
		}
		finally {
	...
	override fun onDraw(canvas: Canvas) {
		...
		if (delayedZoom != null) {
			setZoom(delayedZoom!!.scale, delayedZoom!!.focusX, delayedZoom!!.focusY, delayedZoom!!.scaleType)
			delayedZoom = null
		}
		super.onDraw(canvas)
	}
	...
	fun setZoom(scale: Float, focusX: Float, focusY: Float, scaleType: ScaleType?) {
		...
		if (!onDrawReady) {
			delayedZoom = Zoom(scale, focusX, focusY, scaleType)
			return
		}
	...
	private fun getDrawableWidth(drawable: Drawable?): Int {
		return if (orientationMismatch(drawable) && isRotateImageToFitScreen)
			drawable!!.intrinsicHeight else drawable!!.intrinsicWidth
	}

	private fun getDrawableHeight(drawable: Drawable?): Int {
		return if (orientationMismatch(drawable) && isRotateImageToFitScreen)
			drawable!!.intrinsicWidth else drawable!!.intrinsicHeight
	}
	...
	private fun setState(imageActionState: ImageActionState) {
		this.actionState = imageActionState
	}
	...
	override fun canScrollVertically(direction: Int): Boolean {
		...
		return when {
			imageHeight < viewHeight -> false
			y >= -1 && direction < 0 -> false
	...
	private inner class GestureListener : SimpleOnGestureListener() {
		...
		override fun onDoubleTap(e: MotionEvent?): Boolean {
			...
			if (e != null && isZoomEnabled) {
				...
				if (actionState == ImageActionState.NORMAL) {
					val maxZoomScale = if (doubleTapScale == 0f) DEFAULT_ZOOM_DOUBLE_TAP else doubleTapScale
	...
	private inner class PrivateOnTouchListener : OnTouchListener {
		...
		override fun onTouch(v: View, event: MotionEvent): Boolean {
			...
			if (actionState == ImageActionState.NORMAL || actionState == ImageActionState.DRAG || actionState == ImageActionState.FLING) {
				when (event.action) {
					...
					MotionEvent.ACTION_MOVE -> if (actionState == ImageActionState.DRAG) {
	...
	enum class ImageActionState {
		NORMAL, DRAG, ZOOM, FLING, ANIMATE_ZOOM
	}
	...
	companion object {
		...
		private const val DEFAULT_ZOOM_DOUBLE_TAP = 2f
